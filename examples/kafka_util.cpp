...
bool kafka_util::init_client() {
    ...
    std::string errstr;
    /*设置broker list*/
    std::cout << "brokers_:" << brokers_ << std::endl;
...
void kafka_util::consumer(RdKafka::Message *message, void *opt){
    switch(message->err()){
...
//                   static_cast<const>(message->payload()));
            std::cout << ">>> [" << message->len() << "]" << static_cast<char *>(message->payload()) << std::endl;
...
bool kafka_util::consume(int timeout_ms){
